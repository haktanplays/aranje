...
publish edilir. `setItem` başarısızsa hiçbiri değişmez. Tek istisna deponun
**hiç olmaması**dır: `setItem` çağrılmadı, oturum bellekte çalışır ve bunu
açılışta zaten söyler.
...
ham **798.516 B**, zarf current-only **798.592 B**, current+previous
**1.597.104 B** (≈1,52 MiB, 2,00×). Tarayıcının tahmini kotası güvenlik
garantisi sayılmaz; gerçek `setItem` fail-closed yakalanır.
