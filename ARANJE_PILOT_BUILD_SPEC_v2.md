...
### §13.37 Akustik süreklilik ve devir zarfı (2V-C.4)

#### §13.37.1 Olay bitişikliği akustik süreklilik değildir

**Event zamanlarının bitişik olması, render edilmiş ses enerjisinin bitişik
olduğunu kanıtlamaz.**

2V-C.3'ün shift slide devri için ölçtüğü sayılar — örtüşme 0 s, boşluk 0 s,
varış hatası 0 cent, hedef kendi perdesinde ve kendi anında vuruluyor —
**doğrudur ve geçerliliğini korur**. Ama hepsi scheduler/event düzeyindedir.
Founder aynı devirde «iki ses arasında minik bir boşluk» duydu, ve ikisi
birden doğrudur: bir voice sönerken diğerinin atağı henüz yükselmemiş
olabilir, ve iki olayın *ne zaman başlatıldığı* aritmetiği bunu görmez.

Bu yüzden bu turdan itibaren süreklilik iddiası **yalnız gerçek rendered
PCM'den** kurulur. `eval/expression-fidelity/seam-pcm.ts` üretim offline
renderer'ının yazdığı örnekleri okur: kısa süreli RMS ve tepe zarfı,
kaynağın son enerjili karesi, hedefin ilk enerjili karesi, aradaki sessizlik,
yerel vadi oranı, en büyük örnekten örneğe sıçrama, hedefin transient'i.
Automation listesini yeniden okuyup «ses var» saymaz.

**Analizör bir insanın boşluk duyup duymadığını iddia edemez.** Ölçtüğü şey
akustik vadidir; kartların işini yapmaz, öncesi/sonrası için bir regresyon
kapısı verir.

#### §13.37.2 Tek eşik yeterli değildir; dikiş sınıfları

İlk gerçek koşu bunu kesinleştirdi: legato dikişi çevresindeki enerjinin
**0,81**'ini tutar, yazılı bir es **0**'ını tutar, ama arka arkaya iki kez
çalınmış sıradan bir nota **0,086**'sını tutar — çünkü birincisi sönerken
ikincisinin atağı yükseliyordur, ki penalı bir çalgının yaptığı tam budur.
Legato'yu sürekli sayan tek bir eşik, sıradan çalmayı **bozuk** sayar.

Dikişler bu yüzden **ne tür dikiş oldukları**na göre ölçülür: `joined`
(vurulmayan — legato, hammer-on, pull-off, tutulan nota), `connected`
(vurulan **ve** bağlantısı yazılı — shift slide, şekil slide'ı), `restrike`
(aynı iki nota, arasına hiçbir şey yazılmamış — **referans**, hedef değil),
`broken` (yazılı es — analizörün bulmak **zorunda** olduğu delik).

Eşiklerin tek fixture'a uydurulmasını engelleyen kural: **bağlantısı yazılı
bir dikiş, aynı iki notanın bağlantısız hâlinden ölçülebilir biçimde iyi
olmak zorundadır.** «Bağlı»nın anlamının en zayıf hâli budur ve bağlantısız
yeniden vuruş bir fixture olduğu için karşılaştırma bir zevk değil bir
ölçümdür. Mutlak sessizlik süresi ve tıklama (örnekten örneğe sıçrama)
sınıfa göre gevşemez: delik ve tık her dikişte yanlıştır.

#### §13.37.3 Kayıtların gerçek başlangıcı

Üç paketin bütün buffer'ları üretim yükleyicisiyle çözülüp ölçüldü:
**baştaki dijital sessizlik her dosyada 0,0 ms**'dir. Atlanacak bir şey
yoktur; vendored ses dosyalarına dokunmak için bir sebep de yoktur.

Olan şey **gerçek bir atak rampası**dır ve kayıtlar birbirinden çok farklıdır:
gitar paketinin E4'ü atak tepesinin yarısına **3 ms**'de, A3'ü **31 ms**'de
varır. Dikişin bazı aralıklarda dört kat derin olmasının sebebi budur, ve tek
bir devir sabitinin neden doğru olamayacağının da.

Bu sayılar `src/lib/audio/sample-onset.ts`'te kaynakta durur, çünkü
planlayıcı saftır ve soracak bir buffer'ı yoktur; `profile-samples.mjs`
gerçek çözülmüş sesle karşılaştırır ve uymazsa koşuyu düşürür.

#### §13.37.4 Devir zarfı: kaynak hedefin atağı boyunca devam eder

`src/lib/audio/handoff-envelope.ts` yeniden vurulan her bağlantının tek
politikasıdır. Kaynak artık hedefin onset'inde **durmaz**: onset'i geçen kısa
bir kuyruğu, **hedefin perdesinde**, hedef yükseldikçe sıfıra sönerek sürer.
Gerçek elin yaptığı da budur — perdeye varan parmak teli susturmaz, ardından
gelen pena da eski sesi anında yok etmez.

Kuyruğun uzunluğu **hedefin kendi ölçülmüş atağı + kaynağın kendi release'i**
kadardır, iki uçtan sınırlı ve hedefin notasının üçte biriyle tavanlı — hızlı
bir pasaj asla bir öncekinin altına gömülmez. Onset'teki seviye de atakla
birlikte hareket eder: hemen gürleşen bir kayıt, süründürerek gelen bir
kayıttan daha az köprü ister.

**Yapmadıkları, en az yaptıkları kadar bağlayıcı:** hedefin onset'ini öne
çekmez, yazılı tick'i değiştirmez, buffer'ı erken başlatmaz, ikinci bir
transient/sample eklemez, hedefin atağını gain sıçramasıyla taklit etmez ve
iki sesi uzun süre tam seviyede üst üste bindirmez.

**«Sıfır örtüşme» testi bu turda kaldırıldı** ve yerine geçen iddia daha
katıdır: *perde* tam zamanında varır, seviye o anda belirgin biçimde
düşüktür, ve örtüşme sınırlıdır. Sıfır örtüşme hiçbir zaman amaç değildi;
kesintisiz ses amaçtı.

#### §13.37.5 Tek tel ve şekil aynı politikayı paylaşır

L21 ve L24 aynı cümleyle geri geldi, bu yüzden iki ayrı yama yazılmadı. Bir
şekil, telleri arasındaki **en uzun atağı** alır ve bütün teller aynı takvimle
devredilir: hiçbir tel diğerinden önce bırakılmaz, hedef akor atağı tek bir
onset grubudur, ve tavan tellerin **toplamı** üzerindedir. Her tel yine kendi
kaydını okur; paylaşılan şey soru değil, grubun cevabıdır.

#### §13.37.6 Bu turun çıkış politikası

Bu dikiş için otomatik bir C.5 eğri turu **açılmaz**. Founder'ın cevabı şöyle
haritalanır: **«Olmuş»** mevcut motor için kapatır · **«Kısmen»** pilotu
bloke etmeyen sample/motor cila borcudur · **«Emin değilim»** — PCM kapısı
yeşil ve belirgin tık/boşluk yoksa — mevcut sample bank tavanı olarak
kaydedilir ve eğri ayarı yapılmaz · **«Olmamış» + açık boşluk/tık tarifi**
pilot blokeri olarak kaydedilir, ama yeni bir yama açılmadan önce sample bank
ya da sentez katmanı kararı gerekir.

**Hiçbir test sonucu hiçbir fiziksel sonucu kendiliğinden PASS'a çeviremez.**

### §13.38 Çok eksenli ifade ve teknik aralıkları (2V-D.1)

#### §13.38.1 Tek enum beş soruya cevap veriyordu

`articulation` tek değer tutar ve **dört ayrı soruyu** aynı anda yanıtlar: tel
nasıl vuruldu, perde ne yapıyor, önceki notaya nasıl bağlanıyor, elin çevrede
ne yaptığı. Bu yüzden bir nota ya `accent` ya `bend_full` diyebiliyordu, ikisini
birden değil — ve planlayıcı ilk eşleşen dalda **return** ediyordu.

Üç yeni opsiyonel alan eklendi, hiçbiri göç ettirmiyor:

- **`attack`** — enum'ın «tel nasıl vuruldu» sorusunu yanıtlayan üyeleri, tam
  olarak onlar: `accent`, `ghost`, `dead`, `tapping`, `natural_harmonic`,
  `pinch_harmonic`. `normal` alanın **yokluğudur**. `sustain` ve `staccato`
  burada değildir; onlar notanın ne kadar tutulduğunu söyler ve o soru
  `articulationHold`'undur.
- **`picking`** — tek nota, `down` ya da `up`. `strum`'dan ayrıdır: `strum` bir
  **akorun** nasıl taranıp her sesin zamanda kaydırıldığını söyler.
- **`TechniqueSpan`** — palm mute ve let ring, bir tick aralığı ve bir **tel
  kümesi** üzerinde, section'a bağlı.

#### §13.38.2 Aralık tel taşır, çünkü teknik tel taşır

Bir gitarist elinin kenarıyla alt telleri susturur ve üst tel bunların üzerinde
çınlar. Track-wide bir span, yerine geçtiği nota bazlı bayraktan **daha az**
şey söylerdi — o bayrak zaten teli tek tek ayırabiliyordu.

Üyelik onset'e göre ve yarı açıktır:

    startTicks <= onsetTicks < endTicks

Onset'e göre, örtüşmeye göre değil: span'den önce başlamış ve hâlâ çınlayan bir
nota geriye dönük susturulmaz — el sonra indi ve hiçbir el geriye uzanamaz.
Yarı açık: iki span birbirine değebilir, aradaki anı ikisi birden sahiplenmeden.

Reddedilenler: aynı track'te, zamanda örtüşen **ve** tel paylaşan iki aynı tür
span (`duplicate_span`), ya da bir palm mute ile bir let ring
(`contradictory_span`). Farklı tellerdeki span'ler serbestçe bir arada bulunur —
bütün mesele budur. Değen span'ler **sessizce birleştirilmez**: iki yazan okuyucu
birini taşımak isteyebilir.

Section'lar örtüşemediği için bir span section sınırını **yazılamaz**; bar
sınırını serbestçe geçer.

#### §13.38.3 Resolver beş eksenin tek otoritesidir

`expression-resolver` artık attack'ı kaynağıyla birlikte, picking'i, perdeyi,
bağlantıyı, onset üzerinde tutan teknikleri — bir span'den ya da legacy bir
alandan, **tek eksende**, çünkü «bu nota susturulmuş mu» şarkı nasıl söylemiş
olursa olsun tek sorudur — ve hangi eksenin hoparlöre ulaştığını söyler.

Bir eksene iki cevap **tipli retle** karşılanır ve yalnız suçlu ekseni değil
**bütün eksenleri** düşürür: yarım bir okuma, dosyanın bir şey hoparlörün başka
bir şey söylemesinin yoludur.

#### §13.38.4 Legacy tek anda dönüştürülür, başka hiçbir anda

Açılışta, yüklemede ve alakasız bir editte hiçbir şey göç etmez. Zaten
`palm_mute` taşıyan notaların üzerine bir span çizmek okuyucunun aynı şeyi iki
kez söylemesidir, ve span yazımı değeri **tam olarak kapsadığı** onset'lerde —
doğru track, doğru teller, doğru tick'ler — taşır. Kayıpsızdır çünkü legacy alan
tek değer tutuyordu ve taşınan o değerdir. Kapsanmayan her nota söylediğini
söylemeye devam eder; iki yazım biçimi bir şarkıda doğru biçimde bir arada
bulunur.

#### §13.38.5 Atak bir çıkış değil, bir katmandır

Altı vuruş her biri bitmiş bir plan döndürüyordu, yani ilk eşleşen notanın
yaptığı **tek şey** oydu. Artık bir seviye, bir uzunluk, bir filtre ve —
armonikler için — cent cinsinden bir kayma katıyorlar; planlayıcı bunu perde
ekseninin ürettiğinin **üzerine** uyguluyor.

Cent'ler **toplanır**, çünkü ikisi de tek bir `playbackRate` üzerinde tek bir
sayıya iner: bükülmüş bir pinch armonik 1900 + 200'dür ve toplamak bunun bir
yaklaşığı değil, kendisidir. Bükülmüş armonik telin gerçekten bulunduğu perdeden
başlar ve oraya **tırmanır**.

Preset sayıları değişmedi: aynı ses, daha çok kombinasyonda erişilebilir.

#### §13.38.6 Pena yönü yazılır, çizilir, söylenir — çalınmaz

Gönderilen sample bank'te perde başına **tek kayıt** vardır. Bu yüzden
`picking` `notation_only`'dir ve planı hiçbir şekilde değiştirmez; bir test bunu
doğrudan söyler. Uygulama bunu kendi cümlesiyle kabul eder ve üretemeyeceği bir
farkı ima etmez — iki aynı dosyayı iki teknik diye önizlemeye sunmaz.

### §13.39 D.1'in tamamlanması: bütünlük, sayfa ve Çalım (2V-D.1-C)

#### §13.39.1 8 ms bir yuvarlama değildi

Önceki tur, span'li palm mute ile legacy `articulation: palm_mute` arasındaki
farkı «tick ile saniye arasındaki yuvarlama» diye tarif etti ve `< 0.01`'lik
bir sınırla geçti. **Ölçüm bunu yanlışladı.** Tam zaman çizgisi — yazılı tick,
kapılanmış tick, planlanan saniye, zarf — üç olgu için yan yana yazıldığında
görüldü ki zaman çizgisi legacy enum'ı okuyarak **tick'te** kapılıyor, span'i
göremiyor, ve planlayıcı span'i **ikinci kez** kapılıyordu: `0.92 × 0.45`'e
karşı `0.45`. `palmMuteSeconds` ise mutlak 180 ms tavandır, başka bir iş yapar
ve bu tempoda hiç bağlamaz — yani iki aşamadan biri silinemezdi. Zaman çizgisi
tekniği artık kendisi çözüyor (`techniquesOver`), planlayıcı tekrar kapılamıyor,
ve iki yazım **birebir** eşit: aynı süre, aynı plan, aynı zarf. Gerçek offline
render de aynı şeyi söylüyor: 0.01858 RMS / 107 ms sönüm, susturulmamış notada
0.03989 / 250 ms. İlk okuma `eval/technique-spans/pm-timeline.test.ts`
başlığında kelimesi kelimesine duruyor.

#### §13.39.2 Span bir dikdörtgendir; beş komut aynı aritmetiği kullanır

Copy, paste, move, repeat, delete, restring ve şekil taşıma bir teknik
aralığına aynı iki soruyu sorar: seçim bunun neresine değiyor, ve o parça
nereye gidiyor. Ayrı ayrı yazılsalardı ilk ayrışma «copy'nin koruduğu, move'un
kaybettiği span» olurdu. Bu yüzden seçim **zaman × tel** dikdörtgenidir,
`span-rect` küme aritmetiğini yapar, `span-transform` hangi komutun hangi
aritmetik olduğunu söyler. Notalarını takip edemeyen bir span **komutun
tamamını reddeder** (`span_scope_lost`) — yaklaşık bir yere konmaz. Kesilen bir
span parçalanır ve parçaların kimliği türetilir, kopyalar yeni kimlik alır;
hiçbiri rastgele değildir, yani redo aynı byte'ları yazar. **Üzerinde nota
olmaması orphan değildir:** es üzerindeki bir çalım işareti hâlâ o cümlenin
içindedir.

#### §13.39.3 Beş eksen, tek sayfa

Bir notanın çalınışını dört alan söyleyebiliyordu ve ikisi sayfaya hiç
ulaşmıyordu: yeni `attack` çizilmiyor, span'li palm mute ray çizmiyordu — çünkü
ray legacy enum'ı okuyordu. Yazıp duyup görmemek, yazıp duymamakla aynı
kusurdur. `expression-marks` bütün kelime dağarcığını bir kez adlandırır —
rakamın **üstüne** yazılan (hayalet, ölü nota, doğal armonik), **yanına**
yazılan (vurgu, tapping, pinch armonik, pena yönü), ve **ray** olarak yazılan
(avuç susturma, çınlamaya bırak) — ve glif, işaret ve geometri hep buna sorar.
Ray artık tekniği çözer: span'li mute ile legacy mute **aynı koordinatlarda**
aynı rayı çizer, ve let ring hiç sahip olmadığı rayı kazanır. TabCanvas satır
bütçesi yükseltilmedi.

#### §13.39.4 «Çalım»: üç soru, oyuncunun kelimeleriyle

Model dört bağımsız şey ayırıyor; bir okuyucu eksen düşünmez. Üç grup:
**Vuruş** (nasıl vurdun), **Pena** (pena hangi yöne gitti), **Bölge boyunca**
(elin bu bölüm boyunca ne yapıyor). Etiketler ve «ne işe yarar» cümleleri tek
tabloda durur, bir test hepsini gezer ve hiçbirinin tanımlayıcı, tick ya da
slot içermediğini söyler. Yüzey bir **sheet bölümüdür**, modal değildir:
`fixed inset-0` yoktur ve bir sınır testi bunu dosyanın kendisinde kontrol
eder. Her seçim **uygulanmadan önce ne yapacağını söyler**, ve o cümle apply'ın
çalıştırdığı saf komutun kendisinden gelir — reddi dahil. Önizleme ve red
store'a hiçbir şey yazmaz; apply tek commit yazar. Zaten duran bir bölge işareti
listelenir ve kaldırılabilir: kaldırılamayan teknik denenmeyen tekniktir.

#### §13.39.5 Dışa aktarma ne taşıdığını söyler

Proje dosyası her şeyi taşır ve JSON gidiş-dönüşü byte'ı byte'ına aynıdır. WAV
uygulamanın çıkardığı sesi taşır — artık span'li palm mute dahil, ve bunu
ölçülmüş bir render söylüyor. MIDI notaları ve zamanlamayı taşır; bıraktığı
gölgelemeyi kendi cümlesinde adıyla sayar. **Pena yönü için ayrı bir cümle
eklendi:** yazılır ve duyulmaz. Bu, modelin `notation_only` demesinin okuyucuya
söylenmiş hâlidir.

#### §13.39.6 Yoğunluk ölçüldü, tavan da görüldü

Span yoğunluğu 1× / 4× / 8× — section başına 1, 4 ve 8 span — aynı müzik
üzerinde ölçüldü: index, zaman çizgisi, plan, bir düzenleme ve settle kapısı
ayrı ayrı, p50/p95, **Node/masaüstü** etiketiyle. Rakamlar yoğunlukla düz
kalıyor. Ölçüm sırasında görülen şey kaydedildi: **şema bir section'ı en fazla
8 span ile sınırlıyor** (`songLimits.barsPerSection`), yani 8× bugün yazılabilen
...
#### §13.41.2 L31'in kök nedeni: iki yol aynı notayı aynı kayıttan çalmıyordu

Fixture suçsuzdu — vurgular doğru tick'lerde, iki take yalnız yerlerinde
farklı, plan `1.18`'i birebir uyguluyor.

> **DÜZELTME (2V-D.2 gain parity).** Bu turun ilk yazdığı «expressive yol
> sampler'ın sabit **9,92 dB** üstünde» sonucu **yanlıştır** ve doğrudan
> ölçümle çürütüldü. `neutralParity()` tek notayı iki production yolundan
> geçirip grafiğin her aşamasında tepe okuyor: kaynak, pack trim, track
> channel, master headroom ve ceiling — **beşinde de ×1,000, 0,000 dB**. Gain
> routing'de fark yoktur.

Gerçek mekanik neden **kayıt seçimidir**. Articulation taşımayan nota track'in
paylaşılan `Tone.Sampler`'ı tarafından çalınır ve kaydı kendisi seçer;
taşıyan nota expressive voice'a düşer ve `nearestSample`'a sorar. İki
seçicinin eşitlik kuralı farklıydı: `nearestSample` beraberliği **aşağı**,
`Tone.Sampler._findClosest` yukarı doğru arayarak **yukarı** bozuyordu.

Gitar pack'i E2 A2 C3 E3 A3 C4 E4 tutar; 25 yarım sesin **ikisi** tam ortada
kalır: **D3** ve **D4**. Bu iki perdede aynı yazılı nota, yalnız accent
taşıyıp taşımamasına göre farklı kayıttan ve farklı gerdirmeden çalıyordu.
**L31 tel 1, perde 5 — yani D3 — üzerine yazılmıştır**; c1'in A/B kontrolü de
öyle. Hızlandırılmış kayıt atağına daha erken ulaşır, c1 ölçümü ise onset'ten
itibaren **25 ms** pencere okuyordu ve bu pack tepesine ancak **66 ms**'de
çıkar — yani o pencere seviyeyi değil yükselme hızını ölçtü: D3'te tepe farkı
**0,655 dB**, ilk 25 ms farkı **13,804 dB**.
...
#### §13.41.4 Tek kayıt otoritesi (2V-D.2 gain parity)

Düzeltme telafi eklemez, ayrışmayı kaldırır: `nearestSample` artık
**paylaşılan sampler'ın seçimini** bildirir — eşit uzaklıktaki kayıtlardan
yalnız **yukarıdaki** kazanır. Otorite sampler'dır, tersi değil: düz yol
founder'ın kulakla zaten PASS verdiği yoldur (L1) ve onu kaydırmak, kayıtlı
bir kararı olan sesi yeniden seviyelendirmek olurdu.

Sonrasında 25 yarım sesin **0'ı** ayrışıyor; D3 ve D4 hem notanın tepesinde
hem ilk 25 ms'sinde iki yol arasında **×1,000 — 0,000 dB**.

Ürün kontratı — `accent > plain > ghost` — ölçülerek doğrulanır: kayıtlı bir
perdede (C3) tepe oranları **×1,1800 / ×0,4425**, ilan edilen **×1,18 /
×0,45**; D3'te düzeltmeden önce ×1,2600 / ×0,4723, sonra **×1,1738 /
×0,4477**. `accent ÷ ghost` düzeltmeden sonra **×2,6222 = +8,373 dB**, yani
preset'lerin ilan ettiği oranın kendisi.

Per-kart gain, EQ, ikinci transient/sample, attack preset yeniden
kalibrasyonu, master/limiter değişikliği ve sample asset düzenlemesi yoktur.
...
| **K-65** | **Founder Editor Completion · fiziksel ses otoritesi ve tek kabuk (2V-B.4).** **Fiziksel ses otoritesi kanonik olarak kaydedilmiştir** (founder'ın `s3777h1h8tp95` parçası, gerçek cihaz): L1 Gitar PASS · L2 Bas PASS · L3 Orta başlangıç PASS · L4 Pause/resume INCONCLUSIVE (bloke etmez) · L5 Slide CONDITIONAL PASS (cila borcu) · L6 Vibrato PASS · L7 Hammer-on/pull-off CONDITIONAL PASS (cila borcu) · L8 Power chord ve normal akor PASS · **L9 Uzayan akorun loop dönüşü PASS** · **L10 Hızlı «9–10–9» PASS**. L9 uzayan akorun her loop turunda devam ettiğini, L10 yerel ritim yoğunluğunu ve bağlı çalımı doğrular; bu turda L4/L5/L7 cila çalışması yapılmadı ve L11 oluşturulmadı. **Editörün önceki hâli ölçüldü, hatırlanmadı:** `c11a758`'de bir hücreye dokunmak `FretSheet`'i açıyordu (`fixed inset-0 z-30`, `max-h-[85dvh]`, başlığı «Bar 1 · slot 1 · tel 1») ve grid'in merkezi **altı viewport'un ikisi de dahil on iki ölçümün on ikisinde** hit testine cevap vermiyordu; ilk yüzeyde dokuz nota değeri adı, bir «Süre − ikilik +» step'çisi ve **iki** rakip CTA vardı. **Sonrası: 12/12 `gridHit = grid`, 0 örtücü overlay, 0 jargon, 0 çift adlandırılmış kontrol** (`eval/editor-completion/INVENTORY-before/after.json`). Dock akış içinde bir panel bölgesi kazandı (`max-h-[32dvh]`, kendi içinde scroll, `fixed` yok, scrim yok) ve altı panel — Nota · Akor · Hızlı dizi · Süre · Taşı · Cümle — okurun zaten bildiği dört gruba dağıtıldı. **Dokunuş ve uzun basış aynı kabuğu açıyor**; on altı teknik, retune ve şekil işi `FretSheet`'te kaldı ve artık okurun adıyla istediği ikinci bir adım («Tüm teknikler»). **Önizleme ve uygulama ayrıldı:** `useEditIntent` öneriyi **bellekteki ikinci bir Song** olarak tutuyor, `Dinle` onu production preview motoruna veriyor, porte onu çiziyor, değişen slotlar power-chord kaleminin hayalet makinesiyle amber işaretleniyor (tek slottan listeye genelleştirildi); kanonik Song, defter ve proje deposu `Uygula`'ya kadar dokunulmamış, iptal bir referans bırakmak. **Süre fiillerle**: Uzat · Kısalt · İkiye böl · Üçe böl · Dörde böl · Sonraki notaya kadar · Yarım vuruşa sığdır · Bir vuruşa sığdır · Bu bölümü sıklaştır; ilk yüzeyde tick, PPQ, slot, subdivision ve nota değeri yok, `detailedLength` yalnız «Ayrıntılar» için var. **Kabul koşusu üç gerçek kusur buldu ve üçü de kapatıldı:** (1) yerel sıklaştırma davullu hiçbir ölçüde çalışmıyordu — `bar-regrid` zaten `regridDrums`'a sahipti ve çağrılmıyordu; (2) teklif ile yazma farklı soru soruyordu — ikisi de artık ölçünün **bütün lane'lerinin** dolu anlarını `rhythmAvailability`'ye veriyor, bir ölçünün tek ızgarası olduğu için (K-34); (3) «Üçe böl» var olan bir notaya uygulanamıyordu — `replaceExisting` eklendi ve sınırı yazılı: koşunun kendi aralığında *başlayan* notalar ve bağları gider, aralığa dışarıdan giren bir bağ varsa işlem reddedilir, kırpılmaz. **§9 aynı ölçüde kanıtlandı:** kalabalık bir ölçüde (velocity'li nota, bağlı uzun nota, iki sesli akor, articulation'lı nota, let-ring) yerel sıklaştırmayla bütün ölçü yeniden ızgaralanırken yol dışı ihlal **0**; vacuity kontrolü ve üç negatif kontrol (`changed`/`removed`/`added`) ile birlikte. **Phrase artık görünür:** band staff'ın mevcut üst dolgusuna giriyor, grid yüksekliğinden 0px alıyor, kimlik dört farklı pencerede ve iki zoom'da aynı, grid sınırında yeni phrase oluşmuyor, taşan tarafta chevron var; `Cümle` paneli tutulan aralığı müziğe hiç dokunmadan kalıcı bölgeye çeviriyor ve örtüşmeyi kırpmadan reddediyor. **Akor tek isim otoritesinden geliyor** (`Cm`/`C min`/`C minör`/`Cmoll` tek akor, `CM` majör kalıyor, `F#m`/`Gbm` tona göre yazılıyor) ve süresi ham alan değil niyet. **Taşı iki ayrı niyet**: davul taşınmıyor, yalnız kapsamı şarkı olan bir değişiklik şarkının tonunu yazabiliyor, çalınamayan bir sonuç bütün işlemi reddediyor ve reddin ardından şarkı bayt-eş. **Doğrulama:** `4.934` test / 303 dosya (bu turda +79), tsc/lint/build temiz, `git diff --check` temiz; **Hızlı dizi UI uçtan uca 18/18** (`fast-sequence.mjs`, 412×915, production rotası). **Satır bütçeleri yükseltilmedi**, tek istisna `TabCanvas`'ın `intent-boundary` iğnesi `472 → 478` (phrase bandının dört prop'u; `workspace-boundary`'nin `480` tavanı bağlayıcı) ve `Workspace.tsx` `377/377` ile büyümedi. **Kapsam dışı bırakıldı:** Copilot, pattern/lick kütüphanesi, öneri sıralayıcı, telemetri, LLM sağlayıcı, AI harness, Pro workspace, keyfî 5/7 tuplet, polimetre, L4/L5/L7 ses cilası. **Açık borç:** fiziksel Android/iOS kabulü bu turda yapılmadı; bir ölçünün tek ızgarası olduğu için on altılık içeren bir ölçüye triplet koşu yazılamaz — doğru davranış budur ve okura cümleyle söylenir. | **Haktan editör kabulünü bekliyor** |
| **K-66** | **Bir ölçüde iki ritim, ve ekrana çıkan üç model (2V-B.4 Completion).** **Dürüstlük düzeltmesi önce:** `0c7fa37` B.4'ün **editör kabuğunu ve Hızlı Dizi dilimini** bitirmişti, B.4'ün tamamını değil; phrase çizimi, akor yazma/süre, transpoze ve slide–süre ayrımı o noktada eksikti ve «on akışın onu bitti» cümlesi o noktada doğru değildi. **Izgara bir görüştür, onset'lerin sınırı değil:** straight 1/16 (48 tick) ile onun triplet'i (32 tick) tek `resolution` alanına sığmıyordu ve editör founder'ın kendi riff'ini reddediyordu. Seçilen asgari güvenli çözüm **tam ortak kafestir**: saklanan tek yeni değer **48** (slot 16 tick, `gcd(48,32)`), sixteenth'ler her üçüncü slotta, triplet'ler her ikincide — yuvarlama yok, ölçü uzunluğu değişmiyor, playback planı doğru tick'leri okuyor. **48 sözlükte değildir:** `RESOLUTIONS`, seçiciler, `RHYTHM_PROFILES` ve `MAX_SLOTS_PER_BAR` değişmedi; okunacak ızgara ayrı bir alanda (`Bar.notation`, opsiyonel) durur ve **her** kullanıcıya dönük ızgara sorusu `readingResolution(bar)`'dan geçer — `OfferedResolution` tipi bunu derleyicide zorunlu kılar. 96 (straight 1/32 ile triplet) **bilerek** gönderilmedi; o durumda komut hâlâ `unavailable` der, ve keyfi 5/7 tuplet ile polimetre kapsam dışıdır. **Tarayıcıda bulunan gerçek kusur:** bir ölçüyü kafese yükseltmek çizilen genişliğini üçe katlıyordu — «bütün ölçü başka bir ızgaraya taşındı» görüntüsünün ta kendisi; genişlik artık okunan ızgaradan ölçülüyor ve sütunlar daraltılıyor, üretimde **önce ve sonra 3578px**. **Eski şarkılar göç etmeden açılır** ve `notation` alanını taşımaz; yalnız düzenlenen ölçü yükseltilir. **Seçim renginin hiç tanımlanmamış olduğu bulundu:** `bg-accent`/`border-accent`/`ring-accent` seçim bandında, tutamaklarda, süre önizlemesinde ve her «seçili» çipinde yazılıydı ama `--color-accent` yoktu; Tailwind 4 tanımsız renk için hiçbir şey üretmez ve uyarmaz, dolayısıyla editörün en gür katmanı renksiz çiziliyordu. Steel olarak tanımlandı — palette'in zaten «çalıyor veya seçili» için ayırdığı ton. **Üç katman ayrıldı:** selection dolgulu bant + halka + tutamak; phrase staff'ın kendi 28px üst boşluğunda tek saç teli çizgi ve küçük ad; ghost **tek başına** bronz. Phrase bandının dokunma hedefi **28px**'dir ve bu sessizce geçilmedi: 44px'e çıkarmanın tek yolu notaların üstünü kapatmaktı, aynı iş rafta (`Seçim → Cümle`) tam boy bir kapıya sahiptir, ve geometri koşucusu bu sayıyı kendi eşiğiyle ayrıca raporlar. **Akor paneli domain'in kendi retlerini taşımaya başladı:** `applyChordWrite` zaten sessizce itmiyor, silmiyor, kısaltmıyor ve başka tele taşımıyor **ve hangisini yapmak zorunda kalacağını söylüyordu**; panel hepsini tek bir «Bu süre buraya sığmıyor» cümlesine indiriyordu. Süre iki kayıtta birden söyleniyor: «1 vuruş · dörtlük · 1/4». **Transpoze yazmıyor, öneriyor:** her hareket bir taslak şarkı kurar ve gridde ghost olur; kanonik Song `Uygula`'ya kadar dokunulmaz. Kabul vakaları gerçek şarkılarda ölçüldü — E minor +2 → F# minor, Em → F#m (isim olarak), E5 → F#5, davul birebir aynı, `9h10p9` aynı aralıkta hammer-on + pull-off, slide aynı yön ve mesafe, bölüm/seçim kapsamı global tonu **değiştirmiyor**; standart, drop D, capo, açık şekil, aynı-tel çakışması ve aralık dışı ret ayrı ayrı sınandı — perde clamp'lenmiyor, nota düşürülmüyor, yarısı taşınmış şarkı kalmıyor. **Slide artık «1/4» demiyor:** bağlantı, notanın süresi ve kayma süresi üç ayrı satırdır ve kayma süresi «Otomatik»tir, hiç rakam taşımaz. **Doğrulama:** tam paket **4.993 test / 306 dosya**; hedefli paket 90 test **5 ardışık yeşil**; `fast-sequence.mjs` **18/18**; `inventory.mjs` 12 ölçüm, hepsinde `gridHit=grid`; yeni `flows.mjs` akor/taşı/taşı+ghost/cümle akışlarında 6 viewport × 5 durum = **198/198**; **8 seri mutasyon probe'u, hepsi adıyla kırmızı** (biri ilk turda yeşil geldi ve **probe'un kendi kusuruydu** — kaldırılan dal yedek bir dalla aynı cevabı veriyordu). **Satır bütçeleri yükseltilmedi ve bütçe testi gevşetilmedi;** yeni davranış `ChordShape.tsx` ve `PhraseBand.tsx`'e çıkarıldı. **Playback motorunun karakteri değişmedi ve yeni founder dinleme kartı üretilmedi;** L1–L10 otoritesi yeniden sorulmadı, L11 açılmadı, L4/L5/L7 cila borcu bu turda ödenmedi. Copilot, öneri motoru, LLM, AI harness ve Pro workspace bu turda oluşturulmadı. | **Haktan phrase/akor/transpoze akış kabulünü bekliyor** |
| **K-67** | **Bir notaya üç soru, tek enum yerine (2V-C.1).** `articulation` tek değer tutar ve aynı anda **üç ayrı soruyu** yanıtlar: tel nasıl vuruldu, çalarken perde ne yapıyor, önceki notaya nasıl bağlanıyor. Bu yüzden bir nota ya `bend_full` ya `accent` diyebiliyordu, ikisini birden değil; ve «bük ve yukarıda kal» diyemiyordu, çünkü enum'ın bükmek için tek kelimesi vardı ve o kelime «bük ve geri dön» demekti. **İki isteğe bağlı alan eklendi:** `pitchGesture` (dört bend + tepe vibratosu + yazılı partneri olmayan iki açık kaydırma) ve `connection` (hammer-on, pull-off, **legato slide**, **shift slide**). `targetCents` sayıdır, `half|full` enum'ı değil — sonraki aralık bir sınır değişikliğine mal olur, şema göçüne değil; basit editör hâlâ yalnız 100 ve 200 sunar. `startAfterTarget` yalnız `true` kabul eder: «önce titret, sonra var» bir el hareketi değildir ve artık **yazılamaz**. **Okuma tarafı tek otoriteye bağlandı:** `expression-resolver` üç ekseni ayrı okur; farklı eksenlerde iki değer çakışma değildir (accent + bend sıradan müziktir), **aynı** eksende iki kaynak öncelikle çözülmez, tipli retle karşılanır — sessiz bir kazanan dosyanın bir şey, hoparlörün başka bir şey söylemesidir. **Eski ses çeviriyle değil, kendi yolunu koruyarak korundu:** `bend_full` yazılmış nota legacy işareti olarak çözülür ve planlayıcı onu her zaman kullandığı automation'a yollar; `bend_release` en yakın yeni şekildir ve «en yakın» «aynı» değildir. Sekiz legacy articulation'lı bir ölçü ölçüldü: yarım/tam bend tam 100 ve 200'e varıyor ve **ikisi de sonunda sıfıra dönüyor**, ve yanına bir gesture yazıldığında diğer yedi notanın planı **bayt bayt aynı** kalıyor. **Shift slide bir zincir değildir** ve bu bir tercih değil tanımdır: zincir tam da hedefin *vurulmaması* için vardır, tek ses bütün üyelerin içinden geçer. Shift slide sıradan bir onset'tir, seyahati kendi automation'ında taşır ve kendi onset'inde yazılı perdeye varır; ikisi **aynı** travel eğrisini kullanır, yani fark **yalnız ataktır** — gain hilesi, ikinci scheduler ve ikinci synth yok. Eski `slide` zincir yolunda kalır ve açık `legato_slide` ile byte-eş zincir planı üretir. **Devamlılık için yeni makine gerekmedi, ki asıl mesele bu:** bir bend sıradan perde automation'ıdır, ölçüyü aşan bağlı nota tek gesture taşıyan tek notadır, ve `activeVoicesAt` perdeyi zaten notanın schedule edildiği automation'dan okur. Sınırın yarım ölçü ötesinde duraklatınca `bend` **200 cent'te**, `bend_release` inişte, `prebend` bir slot içinde zaten hedefte; loop wrap aynı plana aynı tick'i sorar ve **byte-eş** cevap verir. Bu sırada `valueAt`'in **iki kopyası** bulundu ve tek kopyaya indirildi. **Notasyon ve erişilebilir dil tek yerden üretiliyor** (`gesture-language`), böylece perdenin yanındaki karakter ile ekran okuyucunun söylediği söz ayrılamaz: dört bend dört ayrı işaret, shift slide `s` ile işaretli çünkü eğik çizgi tek başına «hedef yeniden vuruldu» diyemez, ve cümlelerde cent, enum, slot ya da hata kodu yok. Bir eksene iki cevap veren nota çizilmiyor. **Yazma komutu (`gesture-write`) tek Song ya da hiçbir şey üretir;** on ret yazmadan önce, müzisyen cümlesiyle ve şarkı bayt-eş. Kaydırma yönü **duyulan sesten** gelir, perde numarasından değil — capo ya da düşürülmüş tel büyük perdeyi alçak nota yapabilir. **Üretim UI'ı:** `Çalım → Bend / Kaydır`, iki kelimeyle açılır, ikinci seviye yalnız biri seçilince belirir, slide seçenekleri **yazma komutunun kendisine** sorulur (sunulan çalışır, grileşen komutun kendi cümlesini taşır), ve hiçbir şey yazılmadan önce gridde ghost olur. **Koşarken bir regresyon bulundu ve düzeltildi:** `Çalım` iki panelli olunca bir panel açıkken grubun panel satırı gizleniyor, yani `Taşı` geri çekiliyordu; satır artık iki panelli grupta açık kalıyor. **Doğrulama:** tam süit **5.105 test / 313 dosya**; hedefli paket 129 test **5 ardışık yeşil**; `flows.mjs` 6 viewport × 9 durum = **342/342**; `fast-sequence` 18/18; `inventory` 12 ölçümde `gridHit=grid`; **20 seri mutasyon probe'u, hepsi adıyla kırmızı** — **dördü ilk turda yeşil geldi ve dördü de probe'un kendi kusuruydu**, testler zayıflatılmadı, probe'lar gözlemlenebilir mutasyonlara taşındı. **Satır bütçeleri yükseltilmedi;** yeni davranış yedi küçük adlı modüle çıkarıldı. **Bu turun bütün iddiaları plan, automation ve olay düzeyindedir; sesin nasıl olduğu hakkında hiçbir şey iddia edilmedi.** L11–L16 kartları eklendi ve **founder'dan yalnız o altı kart için kısa bir ses onayı isteniyor** — editör hareketi testi istenmiyor, L1–L10 yeniden sorulmuyor. TechniqueSpan göçü, attack ekseninin kendi alanına taşınması, picking/rake/tremolo/harmonics/ornament ve MPE-MIDI export bilerek yapılmadı. | **Haktan L11–L16 ses onayını bekliyor** |
| **K-68** | **Ölçülmeden hiçbir eğri değiştirilmedi (2V-C.2).** Founder dört kartta memnun kalmadı ya da ölçemedi, bu yüzden ilk iş yeni bir eğri değil bir **ölçüm** oldu: `eval/expression-fidelity` üretim planını okuyup bir hareketin el mi efekt mi olduğuna karar veren anları yazıyor (`BEFORE.md` / `AFTER.md`). **K-67'nin shift slide cümlesi bu turda geçersiz kılındı.** O cümle kodu doğru, müziği yanlış tarif ediyordu: seyahat hedefin kendi automation'ına yazılıyordu, yani hedefin buffer'ı **kaynağın perdesinde** ve hedefin yazılı anında vuruluyor, oradan yukarı kayıyordu. Bu hedefi yeniden vurmak değildir; kaynağı, hedefin zamanında, **geç** vurmaktır — ve kaynak nota hiç kımıldamıyordu (izde: hedef −200 cent'te vuruluyor, yazılı perdesine kendi onset'inden **0,192 s sonra** varıyor). El kaynak notada hareket eder: kaynak hedefin onset'ine kadar uzatıldı, perdesini kendisi olarak duyulacak kadar geç bırakıyor (0,520 s) ve **tam hedef vurulurken** varıyor (0,625 s = handover); hedef kendi perdesinde **düz** vuruluyor. Zincirin zaten sahip olduğu bir kaynak yeniden yazılmıyor — bir tele iki sahip vermek yerine ret korunuyor. Eski sırayı savunan test **zayıflatılmadı, yeni sırayı savunacak şekilde ve daha katı** yazıldı. **Bend'in geri dönüşü bir hareket oldu:** iniş 200 cent'i 0,138 s'de alıyordu, kendi çıkışından (0,211 s) **hızlı** — hiçbir el böyle yapmaz — ve yazılı perdeye notanın **son örneğinde** varıyordu, yani not hiç «döndü» diye duyulmuyordu. Artık iniş çıkıştan yavaş (0,291 s, oran 1,38×) ve **erken bitiyor**: son 0,138 s notanın kendisi olarak duyuluyor. Varış anı bilerek değiştirilmedi. Kısa notada beş aşama tek ortak katsayıyla sıkışıyor: negatif hold yok, notanın dışına yazılan nokta yok, ve oranlar prova hızında korunuyor. **Legacy `bend_half`/`bend_full` ve legacy `slide` kendi yollarında kaldı**, hem «yeni şekle yönlendirilirse kırmızı» hem «iki yol aynılaşırsa kırmızı» testiyle — çit boş bir tarlayı korumuyor. **Pre-bend'in sesi zaten doğruydu, soru yanlıştı:** ilk automation noktası t=0'da hedefe `step`, motor onu `start()` ile aynı anda yazıyor, 64 örnekleme noktasının hepsinde hedefte. L12'nin ölçülememesi karttan geliyordu — «pre-bend tut» ile «pre-bend indir» notanın ilk %88'inde birebir aynı. L18 artık **normal bend ile pre-bend'i** karşılaştırıyor, yani fark ilk karede. **Kayarak çıkma söner, kayarak girme sönmez** ve bu simetri eksikliği kararın kendisidir: giriş, sonra çalmaya devam eden bir notanın **içine** iner; çıkış sesin gitmesidir. Çıkış tam gainle son örneğe kadar gidiyordu (300 cent / 0,118 s), artık seyahat başlarken seviye düşmeye başlıyor ve nota bitiminde %12'ye iniyor — sıfıra değil, çünkü tel zaten susuyor. **Açık kaydırmanın mesafesi üç kelime:** `Kısa / Belirgin / Uzun`, tek yerde sınırlı `approxSemitones`'a bağlı (1/2/4), varsayılan «Belirgin», ham sayı ana yüzeyde yok, gerçek aralık «Daha fazla» altında **söyleniyor**. Nota-nota kaydırmaya sunulmuyor: orada iki yazılı nota zaten aralığı veriyor. Erişilemeyen bir yaklaşım perdesi **tipli retle** karşılanıyor (`slide_off_the_neck`) ve soru **duyulan sesten** soruluyor, yani capo ve alternatif akort cevabı enstrümanla birlikte taşıyor. **C.1 yalnız eklemeyi bitirmişti;** kaldırma ve değiştirme bu turda kapandı: panel notanın **şu an ne yaptığını** söylüyor, iki eksen **ayrı ayrı** kaldırılıyor (bükme kalkarken bağlantı ve accent yerinde kalıyor), nota/perde/konum/tick/süre değişmiyor, olmayan bir şeyi kaldırmak **bayt-eş retle** karşılanıyor, ve fiil yazılana göre konuşuyor — bir kaydırma için «Bükmeyi kaldır» demek okuyucuya yazmadığı bir hareketi anlatmaktır. **Dinleme turu artık founder'ın yetkisini kendi oturum sayacıyla karıştırmıyor:** «Cevaplanmamış: 13/16» hatası, her kartı sayıp cevapları yalnız o tarayıcı oturumunda arayan bir toplamdı. `founder-authority.ts` L1–L16'yı sabit olarak tutuyor (çalışma anında hiçbir şey yazmıyor; L12/L13 **ölçülmedi**, L11 ve L14 **cila gerekiyor**, L14 hiçbir geçişe yükseltilmiyor), `listening-scope.ts` arşivi turdan ayırıyor, ana yüzeyde yalnız L17–L20 var, geçmiş katlanmış ve **cevaplanamaz**. «L11 L11» başlığı ikinci bir tablodandı: `listening-result.ts` L1–L10'u kapsayan bir kısa-ad haritası tutuyor, sonrası için kartın id'sine düşüyordu; harita silindi, kartın kendi `label`'ı tek otorite, ve ikinci bir tablonun geri gelmemesi teste bağlandı. Yorum yazılıp karar verilmemiş kart artık **«ölçülmedi — yorum var»** diyor. **Doğrulama:** tam süit **5.176 test / 316 dosya**; hedefli paket **5 ardışık yeşil**; `panel-geometry` 6 viewport × 2 durum **hepsi temiz** (`gridHit=grid`, yatay taşma 0, 44px altı hedef 0, ham sayı 0, jargon 0); `render-check` **41/41**; **21 seri mutasyon probe'u, hepsi adıyla kırmızı**. Satır bütçeleri yükseltilmedi. **Bu turun iddiaları yine plan ve olay düzeyindedir; sesin nasıl olduğu iddia edilmedi** — founder'dan yalnız **L17–L20**, dört kısa ses kartı isteniyor. | **Haktan L17–L20 ses onayını bekliyor** |
| **K-69** | **Slash zaten yeterliydi (2V-C.3).** C.1 vurarak slide'ı `s/` ile işaretledi; gerekçe «tek başına eğik çizgi hedefin yeniden vurulduğunu söyleyemez» idi. Gerekçe sağlam, sonuç yanlış: tablatürün bunu söyleyecek bir yolu **var** ve o yol slur'u koymamaktır. Tek başına slash hedefi yeniden vurulan bir kaydırmadır; slur altındaki slash tek bir sürekli sestir. Yani fazladan harf, tüm işi zaten tab okuyan birine okunabilir olmak olan bir sayfadaki özel bir uydurmaydı. **İki slide artık aynı çizgiyi çiziyor ve fark yay.** Yay ikinci bir kanal: renderer çiziyor, ekran okuyucu «bağlı» diyor, ikisi de aynı fonksiyona soruyor — sahnedeki şekille kulaktaki söz ayrılamıyor. `s/` isteyen test silinmedi; savunduğu ayrım gerçekten taşıyan kanala taşındı ve «kendi harfimizi uydurma» yasağı kendi testi oldu. **Bunu düzeltirken daha büyük bir boşluk çıktı:** diagonal connector yalnız *legacy* `slide` articulation'ı için çiziliyordu, yani açık `legato_slide` ya da `shift_slide` taşıyan bir nota rakamının yanında bir karakter taşıyor ve porte üzerinde hiç connector göstermiyordu — okuyucu bir şeyin bağlı olduğunu görüyor, neye bağlı olduğunu görmüyordu. Artık iki açık tür de çiziliyor; founder'ın referansındaki `4\2\0` (arka arkaya iki inen bağlı slide) iki gerçek connector olarak render ediliyor. **Şekil slide'ı (double-stop) türetiliyor, saklanmıyor.** Kontrat bunu neredeyse zaten söyleyebiliyordu — her biri connection taşıyan iki nota *neredeyse* aynı şey. Neredeyse, ve fark asıl mesele: iki bağımsız slide farklı mesafe, farklı yön ve farklı varış anı taşıyabilir, ve birini kaldırmak ötekini bırakır — bir el hareketinin yarısını silmek okuyucunun kastedebileceği bir şey değil. `shape-slide.ts` gesture'ı notalardan türetiyor; argüman dosyada, kanıt testte: bir şekil kendi tellerinden biri düzenlendikten sonra **doğru biçimde yeniden türetiliyor**, yani «düzenle ve kaldır hepsine birlikte uygulanır» modelin sonucu, UI'ın hatırlaması gereken bir şey değil. Zaten ilk kaynağın tuttuğu bir olgu için ikinci bir doğruluk kaynağı, yalnızca ikisinin çelişmesini kazandırır. Yazma komutu **ya bütün teller ya hiçbiri**: aday Song her telden geçiriliyor, sonuç hâlâ şekil mi diye şekil modeline soruluyor, ve ancak ondan sonra settle ediliyor — gripteki açık tel ya da farklı mesafe kateden tel, çağıranın Song'u dokunulmamış hâldeyken bütün gesture'ı reddediyor. **«Vurarak biraz kusurlu» bir ana dair bir cümleydi, o yüzden önce o an ölçüldü.** `handoff.ts` §3'ün sorduğu neredeyse her şeyi eledi: sesler üst üste binmiyor (0 s), boşluk yok (0 s), glide hedefe tam varıyor (0 cent hata), hedef kendi perdesinde vuruluyor (0 cent), ve iki atak var — her birine bir tane. Bulduğu şey bir **genlik basamağı**: kaynak ~105 ms kaydıktan sonra **tam seviyede** duruyor — ve anında duruyor, çünkü kuyruğunu şekillendiren bir şey yok — ve tam o örnekte ikinci bir tam seviyeli atak başlıyor. Aralarında süreksizlik olan iki tam genlikli olay bir tıktır, ve kendini duyurmayacak kadar küçük bir tık «biraz kusurlu»nun sesidir. Fiziksel olarak da yanlış: perdeye varan parmak teli söndürür, ardından gelen pena tekrar söndürür; eski ses vuruşun *içine* söner. Kaynak artık seyahati boyunca sönüp **%45'te** varıyor — ölçülmüş, sıfıra değil, çünkü kulağın elin hedefe *ulaştığını* duyması gerekiyor ve o varış gesture'ın kendisi. Hedefe dokunulmadı: atağı kartın konusu, onu yumuşatmak şikâyeti konusunu silerek cevaplamak olurdu. Zincirlere de dokunulmadı. **Ölçerken ikinci bir kusur çıktı:** «Kısa» ve «Belirgin» 100 ve 200 cent'i **aynı 120 ms**'de alıyordu — nota-nota tabanı — yani uzun olanda el iki kat hızlı hareket ediyordu; «Uzun» ise saniyede 2222 cent, ki orası slide'ın parmak olmaktan çıkıp whammy koluna dönüştüğü yer. Aynı hareketin uzağa gitmiş hâli olduğunu iddia eden üç kelime üç ayrı hareketti. Taban yanlış yerden ödünçtü: nota-nota slide'ın varışı müzikle sabittir, açık slide'ın partneri yoktur ve borcu tutarlılıktır. Kendi hızını alınca üçü de **saniyede 1429 cent**te seyahat ediyor ve yalnız ne kadar gittiklerinde ayrılıyorlar; kısa nota hâlâ seyahati kısaltıyor, taşırmıyor. **Çok telli playback senkron:** iki telli şekil aynı anda ayrılıyor, aynı anda varıyor, aynı policy ile devrediyor ve aynı anda bitiyor — flam yok; bağlı şekil hiçbir hedefi vurmuyor, vurarak şekil ses başına **tam bir** atak veriyor ve hiçbir yerden üçüncü bir ses çıkmıyor. **UI ayrı bir editör açmıyor:** mevcut `Çalım → Kaydır` akışı seçimden karar veriyor — tek ses eskiden beri olan tek-nota slide'ı, iki veya daha fazlası «Bağlı / Vurarak» ve `"2 tel birlikte yukarı kayacak"` özeti. Ham tel indeksi, cent, slot ya da bizim ürettiğimiz bir id yok; tam ekran sheet ve `fixed inset-0` yok. **Doğrulama:** tam süit **5.245 test / 320 dosya**; hedefli paket **5 ardışık yeşil**; `panel-geometry` 6 viewport × 2 durum **hepsi temiz**; `render-check` **41/41** (L21–L24 dördü de çalıyor); **40 seri mutasyon probe'u, hepsi adıyla kırmızı** — biri ilk turda yeşil geldi ve sebebi dürüstçe kaydedildi: tek telde perde farkı ile yarım ses farkı **aynı sayıdır**, yani capo ve alternatif akort bu iki okumayı ayıramaz; ayıran şey yazılı perde ile yazılı sesin çeliştiği bir Song, ve probe'un arkasındaki fixture artık onu kuruyor. Satır bütçeleri yükseltilmedi. **İddialar yine plan, notasyon ve olay düzeyinde; sesin nasıl olduğu iddia edilmedi** — founder'dan yalnız **L21–L24** isteniyor. | **Haktan L21–L24 ses onayını bekliyor** |
