 ...
 *
 * Read down the list: fewer big jumps beats everything; then less overshoot
 * past the threshold; then less hand travel; then a narrower chord; then less
 * wandering across the strings; then a lower position on the neck; then less
 * total fretting; and finally the canonical signature, so a tie is broken by
 * the music rather than by whichever branch the loop happened to reach first.
 */
...
function extend(
  ...
  return {
    cost: [
      ...
      state.cost[1] + excess,
      state.cost[2] + travel,
      state.cost[3] + voicing.span,
      state.cost[4] + centerMove,
      state.cost[5] + voicing.maxPhysicalFret,
      state.cost[6] + voicing.totalPhysicalFret,
      extendSignature(state.cost[7], voicing),
    ],
...
function skip(state: BeamState, resetBefore: boolean): BeamState {
  return {
    cost: [
      ...
      state.cost[5],
      state.cost[6],
      extendSignature(state.cost[7], null),
    ],
  ...
  input.onsets.forEach((onset, index) => {
    ...
    for (const state of beam) {
      ...
      for (const voicing of voicings) {
        next.push(extend(state, voicing, input.maxShift, resetBefore));
      }
    }
